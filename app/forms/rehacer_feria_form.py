...
class RehacerFeriaForm(forms.ModelForm):
    class Meta:
        model = Feria
        fields = "nombre", "categoria", "fecha_inicio", "fecha_fin", "ubicacion", "capacidad_puestos", "activa"
        widgets = {
            "nombre": forms.TextInput(attrs={"class": "form-control"  , "placeholder": "Nombre de la feria", "required": True , 'readonly': 'readonly'}),
            "categoria": forms.Select(attrs={"class": "form-select", 'readonly': 'readonly',"required": True}),
    ...
    def clean_edicion(self):
        ...
        if edicion and edicion <= 0:
            raise forms.ValidationError("La edición debe ser mayor o igual a 0.")
        return edicion
